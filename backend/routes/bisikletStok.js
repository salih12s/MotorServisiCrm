...
// Tüm bisiklet/e-bike stok kayıtlarını getir
router.get('/', async (req, res) => {
  try {
    ...
    const [result, summaryResult] = await Promise.all([
      ...
      pool.query(
        `SELECT COUNT(*)::int AS total,
         ...
         FROM bisiklet_stok ${whereClause}`,
        search ? [searchParam] : []
      ),
    ]);
    ...
    console.error('Bisiklet stok listesi hatası:', error);
...
router.get('/ara', async (req, res) => {
  try {
    ...
    console.error('Bisiklet stok arama hatası:', error);
    res.status(500).json({ message: 'Sunucu hatası' });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM bisiklet_stok WHERE id = $1', [req.params.id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'Stok kaydı bulunamadı' });
    }
    res.json(result.rows[0]);
  } catch (error) {
    console.error('Bisiklet stok detayı hatası:', error);
    res.status(500).json({ message: 'Sunucu hatası' });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO bisiklet_stok (stok_kodu, stok_adi, giren_miktar, cikan_miktar, mevcut, birimi, alis_fiyati, satis_fiyati, envanter_degeri, resim, resimler, aciklama)
    ...
    console.error('Bisiklet stok oluşturma hatası:', error);
    res.status(500).json({ message: 'Sunucu hatası' });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    console.error('Bisiklet stok güncelleme hatası:', error);
    res.status(500).json({ message: 'Sunucu hatası' });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM bisiklet_stok WHERE id = $1 RETURNING *', [id]);
    ...
    console.error('Bisiklet stok silme hatası:', error);
    res.status(500).json({ message: 'Sunucu hatası' });
  }
});
