      ...
      <Table size="small" sx={{ minWidth: 620 }}>
        ...
        <TableBody>{items.map((motor, index) => (
          <TableRow key={motor.id} sx={{ '& td': { borderBottom: index < items.length - 1 ? '1px solid rgba(54,197,211,.08)' : 'none' }, '&:hover': { background: 'rgba(54,197,211,.06)' } }}>
            ...
            <TableCell sx={{ py: 1.5 }}><Typography sx={{ fontWeight: 700, color: '#fff' }}>{motor.name}</Typography><Typography variant="caption" color="rgba(255,255,255,.65)" sx={{ display: { xs: 'block', lg: 'none' }, mt: .5 }}>Fiyat: {money(motor.installmentPrice)}</Typography></TableCell>
            <TableCell sx={{ display: { xs: 'none', lg: 'table-cell' } }}>{motor.installmentPrice ? <Typography sx={{ color: '#fff', fontWeight: 600 }}>{money(motor.installmentPrice)}</Typography> : <Chip label="Fiyat Sorunuz" size="small" sx={{ color: '#36C5D3', border: '1px solid rgba(54,197,211,.3)' }} />}</TableCell>
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#02080f', color: '#fff', overflowX: 'hidden' }}>
      ...
      <Box sx={{ pt: { xs: '72px', sm: '80px', md: '88px' }, pb: { xs: 4, md: 5 }, background: 'linear-gradient(180deg, rgba(4,167,184,.18) 0%, rgba(2,8,15,0) 100%)', borderBottom: '1px solid rgba(54,197,211,.12)' }}>
        <Container maxWidth="lg" sx={{ pt: { xs: 4, md: 5 } }}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={2} mb={1.5}>
            ...
            <Box><Typography component="h1" sx={{ fontWeight: 900, fontSize: { xs: '1.8rem', sm: '2.4rem', md: '3rem' }, background: 'linear-gradient(135deg,#04A7B8,#7be3ee)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1.1 }}>Falcon Fiyat Listesi</Typography><Typography sx={{ mt: .5, color: 'rgba(255,255,255,.6)' }}>2026 model yılı · Güncel fiyatlar</Typography></Box>
          </Stack>
        </Container>
      </Box>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
        <Box sx={{ borderRadius: 3, border: '1px solid rgba(54,197,211,.2)', overflow: 'hidden', mb: { xs: 5, md: 7 } }}>
          <Box sx={{ px: 3, py: 2.5, background: 'linear-gradient(135deg,rgba(4,167,184,.18),rgba(54,197,211,.10))', borderBottom: '1px solid rgba(54,197,211,.15)', display: 'flex', alignItems: 'center', gap: 1.5 }}><Box sx={{ width: 36, height: 36, borderRadius: 1.5, background: 'linear-gradient(135deg,#04A7B8,#36C5D3)', display: 'grid', placeItems: 'center' }}><CreditCardIcon /></Box><Box><Typography fontWeight={800}>Taksit Seçenekleri</Typography><Typography variant="caption" color="rgba(255,255,255,.5)">Kredi kartı ile ödeme seçenekleri</Typography></Box></Box>
        ...
        <Box sx={{ mt: 6, p: 3, borderRadius: 3, background: 'rgba(54,197,211,.05)', border: '1px solid rgba(54,197,211,.15)' }}><Typography fontWeight={700} color="#36C5D3" mb={1}>Önemli Bilgi</Typography><Typography sx={{ fontSize: '.82rem', color: 'rgba(255,255,255,.55)', lineHeight: 1.7 }}>Listelenen tutarlar Falcon tarafından yayımlanan güncel taksitli satış fiyatlarıdır. Stok durumu için mağazamızla iletişime geçiniz.</Typography></Box>
      </Container>
      <SiteFooter />
    </Box>
  );
