...
function PaymentDetailsSection({ formData, setFormData, handleChange }) {
  ...
  const paymentField = (name, label, color) => <TextField disabled={hasCariCollection} fullWidth size="small" name={name} label={label} type="number" value={formData[name]} onChange={handleChange} inputProps={{ min: 0, step: .01 }} InputProps={{ startAdornment: <InputAdornment position="start" sx={{ color }}>₺</InputAdornment> }} />;
  ...
  return (
    <FormSectionCard icon={<ReceiptIcon />} iconBg="success.lighter" iconColor="success.main" title="Ödeme Detayları">
      <FormControl fullWidth size="small" sx={{ mb: 1.5 }}>
        ...
        <Select disabled={hasCariCollection} name="odeme_sekli" label="Ödeme Türü" value={formData.odeme_sekli} onChange={(event) => setFormData((prev) => ({ ...prev, odeme_sekli: event.target.value, nakit_tutar: '', kart_tutar: '', havale_tutar: '' }))}>
          <MenuItem value="nakit">Nakit</MenuItem><MenuItem value="kart">Kart</MenuItem><MenuItem value="havale">Havale / EFT</MenuItem><MenuItem value="karisik">Karışık / Çoklu Ödeme</MenuItem>
        </Select>
      </FormControl>
      {formData.odeme_sekli === 'karisik' && <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mb: 1.5 }}>
        {paymentField('nakit_tutar', 'Nakit', '#16a34a')}{paymentField('kart_tutar', 'Kart', '#2563eb')}{paymentField('havale_tutar', 'Havale', '#9333ea')}
      </Stack>}
